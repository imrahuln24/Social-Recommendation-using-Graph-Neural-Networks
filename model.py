...
class Recommender:
	def __init__(self, sess, handler):
		...
		# In __init__ or prepareModel
		self.lat_attn_user = NNs.defineParam('lat_attn_user', [1], reg=True)
		self.lat_attn_item = NNs.defineParam('lat_attn_item', [1], reg=True)
		self.preds_one = []  
		self.final_one = [] 

		actual_users = self.handler.trnMat.shape[0]
		actual_items = self.handler.trnMat.shape[1]
		print(f'Actual USER: {actual_users}, ITEM: {actual_items}')
		self.metrics = dict()
	...
	def makeTimeEmbed(self):
		...
		return timeEmbed
	def messagePropagate(self, srclats, mat, type='user', graph_idx = 0):
		# Preserve original dimensions from dataset
		max_user_idx = self.handler.trnMat.shape[0] - 1
		max_item_idx = self.handler.trnMat.shape[1] - 1

		# Safe index handling
		safe_users = tf.clip_by_value(self.users, 0, max_user_idx)
		safe_items = tf.clip_by_value(self.items, 0, max_item_idx)

		# Message propagation with dimension checks
		srcNodes = tf.squeeze(tf.slice(mat.indices, [0, 1], [-1, 1]))
		srcNodes = tf.clip_by_value(srcNodes, 0, srclats.shape[0] - 1)
		srcEmbeds = tf.nn.embedding_lookup(srclats, srcNodes)

		tgtNodes = tf.squeeze(tf.slice(mat.indices, [0, 0], [-1, 1]))
		lat = tf.math.unsorted_segment_sum(srcEmbeds, tgtNodes, mat.dense_shape[0])

		# Residual connection with learnable attention weight
		if type == 'user':
			lat_attn = tf.nn.sigmoid(self.lat_attn_user)
			lat = lat * lat_attn + tf.nn.embedding_lookup(srclats, safe_users) * (1 - lat_attn)
			lat = tf.nn.embedding_lookup(lat, safe_users)
		else:
			item_emb = tf.nn.embedding_lookup(self.iEmbed[graph_idx], safe_items)  # Directly access item embeddings
			lat_attn = tf.nn.sigmoid(self.lat_attn_item)
			lat = lat * lat_attn + item_emb * (1 - lat_attn)
			lat = tf.nn.embedding_lookup(lat, safe_items)
	...
	def edgeDropout(self, mat):
		def dropOneMat(mat):
			...
			# Calculate density
			nnz = tf.cast(tf.size(values), tf.float32)
			total_size = tf.cast(shape[0] * shape[1], tf.float32)
			density = nnz / total_size
			
			# Adaptive dropout based on density
			keep_prob = tf.minimum(0.8, 0.2 + density * 0.6)
			
			# Use adaptive keep_prob instead of fixed keepRate
			newVals = tf.nn.dropout(tf.cast(values, dtype=tf.float32), keep_prob)
	...
	def ours(self):
		...
		self.preds_one.clear()  
		self.final_one.clear()
		...
		self.users = tf.range(actual_users)
		self.items = tf.range(actual_items)
		...
		for k in range(args.graphNum):
			...
			for i in range(args.gnn_layer):
				...
				a_emb1 = self.messagePropagate(embs0[-1], self.edgeDropout(self.subTpAdj[k]), 'item', k)
		...
		# GRU for interval-level sequential modeling
		def gru_cell():
		...
		# Multi-head self-attention for interval-level features
		...
		multihead_user_vector = self.multihead_self_attention0.attention(tf.contrib.layers.layer_norm(user_vector_tensor))
		multihead_item_vector = self.multihead_self_attention1.attention(tf.contrib.layers.layer_norm(item_vector_tensor))
		...
		# Instance-level self-attention sequence modeling
		self.multihead_self_attention_sequence = [MultiHeadSelfAttention(args.latdim, args.num_attention_heads) for _ in range(args.att_layer)]
		...
		# Personalized SSL weight generation
		user_weight = []
		...
		# Enhanced SSL loss with contrast between reliable and noisy samples
		sslloss = 0
		for i in range(args.graphNum):
			...
			# Main SSL loss
			sslloss += tf.reduce_sum(tf.maximum(0.0, 1.0 - S_final * (posPred - negPred)) * 0.8)

			# Additional contrastive term for reliable/noisy samples
			reliable_mask = tf.cast(tf.greater(posweight_final, 0.6), tf.float32)
			noisy_mask = tf.cast(tf.less(negweight_final, 0.4), tf.float32)
			sslloss += tf.reduce_sum(reliable_mask * noisy_mask * 0.2)
	...
	def prepareModel(self):
		...
		# Add in prepareModel()
		self.uEmbed = NNs.defineParam('uEmbed', [args.graphNum, args.user, args.latdim], reg=True)
		self.iEmbed = NNs.defineParam('iEmbed', [args.graphNum, args.item, args.latdim], reg=True)
	...
	def trainEpoch(self):
		for k in range(args.graphNum):
			density = self.handler.subMat[k].count_nonzero() / (args.user * args.item)
			print(f"Partition {k}: Density={density:.6f}, Shape={self.handler.subMat[k].shape}")
		num = args.user
